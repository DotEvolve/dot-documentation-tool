...
const functionSignatureArbitrary = fc.record({
  ...
  sideEffects: fc.option(
    fc.array(sideEffectArbitrary, { minLength: 0, maxLength: 3 }),
    { nil: undefined },
  ),
});
    ...
    fc.assert(
      fc.property(codeElementArbitrary, (element) => {
        ...
        if (element.signature!.parameters.length > 0) {
          ...
          expect(jsdoc.params!.length).toBe(
            element.signature!.parameters.length,
          );
  ...
  it("should document all thrown errors", () => {
    fc.assert(
      fc.property(codeElementArbitrary, (element) => {
        ...
        const errorGeneratingSideEffects =
          sideEffects?.filter(
            (se) =>
        ...
        if (hasExplicitThrows || hasSideEffectErrors) {
          ...
          if (hasExplicitThrows) {
            throws!.forEach((errorType) => {
              const documented = jsdoc.throws!.some(
                (doc) => doc.type === errorType,
              );
  ...
  it("should maintain completeness for functions with all elements", () => {
    fc.assert(
      fc.property(codeElementArbitrary, (element) => {
        ...
        if (
          element.signature!.returnType &&
          element.signature!.returnType !== "void"
        ) {
          expect(jsdoc.returns).toBeDefined();
        }
        ...
        const hasThrows =
          element.signature!.throws && element.signature!.throws.length > 0;
        const errorGeneratingSideEffects =
          element.signature!.sideEffects?.filter(
            (se) =>
              se.type === SideEffectType.DATABASE ||
              se.type === SideEffectType.API_CALL ||
              se.type === SideEffectType.FILE_IO,

...
const jsdocArbitrary = fc.record({
  ...
  params: fc.option(
    fc.array(paramDocArbitrary, { minLength: 0, maxLength: 8 }),
    { nil: undefined },
  ),
  returns: fc.option(returnDocArbitrary, { nil: undefined }),
  throws: fc.option(
    fc.array(throwsDocArbitrary, { minLength: 0, maxLength: 5 }),
    { nil: undefined },
  ),
...
function hasValidTagFormat(jsdocString: string): boolean {
  ...
  for (const line of tagLines) {
    ...
    if (tagName === "param") {
      ...
      if (
        !trimmed.match(
          /^\*\s+@param\s+\{[^}]+\}\s+(\[)?[a-zA-Z_][a-zA-Z0-9_=\[\]"{}]*(\])?\s*(-\s+.+)?$/,
        )
      ) {
        return false;
      }
  ...
  it("should handle multi-line descriptions correctly", () => {
    fc.assert(
      fc.property(
        fc.record({
          description: fc
            .array(fc.lorem({ maxCount: 10 }), { minLength: 2, maxLength: 5 })
            .map((lines) => lines.join("\n")),
          params: fc.option(fc.array(paramDocArbitrary, { maxLength: 2 }), {
            nil: undefined,
          }),
        }),
        (jsdoc) => {

...
import { CodeElement, CodeElementType, FunctionSignature, Parameter } from '../../src/types';
...
describe('Property 3: Parameter Constraint Documentation', () => {
  ...
  const constrainedParameterArb = fc.record({
    name: fc.stringOf(fc.constantFrom('a', 'b', 'c', 'd', 'e'), { minLength: 3, maxLength: 10 }),
    type: fc.constantFrom('number', 'string', 'boolean', 'object'),
    optional: fc.boolean(),
    constraint: fc.oneof(
      fc.record({ type: fc.constant('range'), min: fc.integer(), max: fc.integer() }),
      fc.record({ type: fc.constant('enum'), values: fc.array(fc.string(), { minLength: 2, maxLength: 5 }) }),
      fc.record({ type: fc.constant('nullable'), allowNull: fc.boolean(), allowUndefined: fc.boolean() })
    )
  });

  // Generator for function signatures with constrained parameters
  const functionWithConstraintsArb = fc.record({
    name: fc.stringOf(fc.constantFrom('a', 'b', 'c', 'd', 'e'), { minLength: 5, maxLength: 15 }),
    parameters: fc.array(constrainedParameterArb, { minLength: 1, maxLength: 3 }),

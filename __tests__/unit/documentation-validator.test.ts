...
import {
  ...
  Glossary,
} from "../../src/validators/DocumentationValidator";
import {
  JSDoc,
  FunctionSignature,
  ValidationErrorType,
 ...
 */`;
      ...
      const errors = validator.validateSyntax(invalidJSDoc);
      expect(errors.length).toBeGreaterThan(0);
      expect(errors.some((e) => e.message.includes("Invalid JSDoc tag"))).toBe(
        true,
      );
    });
 ...
 */`;
      ...
      const errors = validator.validateSyntax(invalidJSDoc);
      expect(errors.length).toBeGreaterThan(0);
      expect(errors.some((e) => e.message.includes("Unmatched braces"))).toBe(
        true,
      );
    });
 ...
 */`;
      ...
      expect(errors.length).toBeGreaterThan(0);
      expect(
        errors.some((e) => e.message.includes("missing type annotation")),
      ).toBe(true);
    });
 ...
 */`;
      ...
      expect(errors.length).toBeGreaterThan(0);
      expect(
        errors.some((e) => e.message.includes("missing parameter name")),
      ).toBe(true);
    });
 ...
 */`;
      ...
      expect(
        errors.some((e) =>
          e.message.includes("@returns is missing type annotation"),
        ),
      ).toBe(true);
    });
 ...
 */`;
  ...
  describe("validateCompleteness", () => {
    ...
    it("should pass when throws are documented", () => {
      const jsdoc: JSDoc = {
        description: "A function",
        params: [],
        throws: [{ type: "Error", condition: "When something fails" }],
      };
  ...
  describe("checkRedundancy", () => {
    ...
    it("should not flag meaningful descriptions", () => {
      const jsdoc: JSDoc = {
        description:
          "Retrieves user data from the database and validates permissions before returning",
        params: [],
      };
    ...
    it("should not flag descriptions with different context", () => {
      const jsdoc: JSDoc = {
        description:
          "Validates the authentication token by checking expiration and signature",
        params: [],
      };
 ...
 */`;
      ...
      expect(
        result.warnings.some(
          (w) => w.type === ValidationWarningType.TERMINOLOGY,
        ),
      ).toBe(true);
    });

...
import { generateJSDoc, formatJSDoc } from '../../src/generators/JSDocGenerator';
...
describe('formatJSDoc', () => {
  ...
  it('should format optional parameters with default values', () => {
    const jsdoc: JSDoc = {
      ...
      params: [
        { name: 'name', type: 'string', description: 'User name' },
        { name: 'greeting', type: 'string', description: 'Greeting message', optional: true, defaultValue: '"Hello"' }
      ]
    };
    ...
    expect(formatted).toContain(' * @param {string} name - User name');
    expect(formatted).toContain(' * @param {string} [greeting="Hello"] - Greeting message');
  });
  ...
  it('should format a JSDoc with throws documentation', () => {
    const jsdoc: JSDoc = {
      ...
      throws: [
        { type: 'Error', condition: 'If denominator is zero' }
      ]
    };
  ...
  it('should format a JSDoc with multiple throws', () => {
    ...
    expect(formatted).toContain(' * @throws {ValidationError} If input validation fails');
    expect(formatted).toContain(' * @throws {DatabaseError} If database operation fails');
  });
  ...
  it('should format a complete JSDoc with all elements', () => {
    const jsdoc: JSDoc = {
      ...
      params: [
        ...
        { name: 'options', type: 'object', description: 'Processing options', optional: true, defaultValue: '{}' }
      ],
      returns: {
        type: 'Promise<User>',
